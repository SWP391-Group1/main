...
public class MedicalMethodDAOImpl extends AbstractDAO<MedicalMethod>
    implements MedicalMethodDAO {

  @Override
  public Long save(MedicalMethod medicalMethod) {
    String sql =
        "INSERT INTO medical_method (result_id, name, diagnosis, conclude, created_stamp " +
            " VALUES (?, ?, ?, ?, ?)";
  ...
  public MedicalMethod update(MedicalMethod medicalMethod) {
    String sql =
        "UPDATE medical_method SET result_id=?, name=?, diagnosis=?, conclude = ?, " +
  ...
  public MedicalMethod findById(Long id) {
    ...
    List<MedicalMethod> medicalMethods =
        query(sql, new MedicalMethodMapper(), id);
    return CollectionUtils.isEmpty(medicalMethods) ? null :
        medicalMethods.get(0);
  }
  ...
  public List<MedicalMethod> findByResultId(Long resultId) {
    ...
    List<MedicalMethod> medicalMethods =
        query(sql, new MedicalMethodMapper(), resultId);
