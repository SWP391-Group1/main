...
<%--                                        <label for="diagnosis" style="font-weight: bold" class="col-form-label">Diagnosis</label>--%>
                                        <div class="form-group">
                                            ...
                                            <input required id="name" name="name" readonly
                                                   value="${requestScope.resultSelected.name}"
                                                   class="form-control"/>
                                        </div>
                                        <c:if test="${requestScope.scheduleSelected.type eq 'HEALTH_CARE'}">
                                            <div class="form-group">
                                                <label for="diagnosis" class="col-form-label">Diagnosis</label>
                                                <textarea required id="diagnosis" name="diagnosis" type="text" rows="3" readonly
                                                          class="form-control">${requestScope.resultSelected.diagnosis}</textarea>
                                            </div>
                                        </c:if>
                                        <c:if test="${requestScope.scheduleSelected.type eq 'TEST_COVID'}">
                                            <div class="form-group">
                                                <label  class="col-form-label">Time test</label>
                                                <p class="card-text"
                                                   style="text-align: left;margin-left: 10px;color: #000000">
                                                    <fmt:formatDate pattern="dd-MM-yyyy HH:mm a"
                                                                    value="${requestScope.resultSelected.createdStamp}"/>
                                                </p>
                                            </div>
                                        </c:if>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea required id="conclude" name="conclude" type="text" rows="3" readonly
                                                      class="form-control">${requestScope.resultSelected.conclude}</textarea>
                                        </div>

                                        <c:if test="${requestScope.scheduleSelected.type eq 'HEALTH_CARE'}">
                                        <div id="method-panels">
                                            <c:forEach var="method" items="${requestScope.methodsSelected}">
                                                <div style='margin-top: 20px;margin-bottom: 20px'> +
                                                    <label style='font-weight: bold'>Method ${ method.name}</label>
                                                    <div class='form-group'>Diagnosis<textarea required type='text'
                                                                                               readonly
                                                                                               rows='3'
                                                                                               class='form-control'
                                                                                               name='diagnosis'>${method.diagnosis}</textarea>
                                                    </div>
                                                    <div class='form-group'>Conclude<textarea required type='text'
                                                                                              readonly
                                                                                              rows='3'
                                                                                              class='form-control'
                                                                                              name='conclude'>${method.conclude}</textarea>
                                                    </div>
                                                    <c:if test="${method.image ne null}">
                                                        <img id="blah" width="200" height="200" src="<c:url value="${method.image}"/>" alt="your image" />
                                                    </c:if>
                                                </div>
                                            </c:forEach>
                                        </div>
        ...
        for (var i = 0; i < choosedMethods.length; i++) {
            if (choosedMethods[i].checked === true) {
                ...
                methodPanels.innerHTML = methodPanels.innerHTML +=
                    ...
                    "<div class='form-group'>Images<input type='file' onchange='readURL(this," + i + ")' multiple='multiple' accept='image/*' rows='3' class='form-control' name='image'" + "></input></div>" +
